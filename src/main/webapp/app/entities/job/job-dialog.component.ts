...
export class JobDialogComponent implements OnInit {
    ...
    filterCost: number;
    graduationDateAdded;
    graduationToDateAdded;
    graduationFromDateAdded;
    isNew;
    ...
    jobFilter: JobFilter;
    prevJobCost;
    prevOriginalJobCost: number;
    jobCostDifference;
    absoluteJobCostDifference: number;
    paymentType: string;
    amountOutstanding;
    ...
    prevEmploymentTypeCost;
    prevJobTypeCost;
    currentEmploymentTypeCost;
    currentJobTypeCost;
    prevNoOfApplicants: number;
    noOfApplicantsIncreased;
    noOfApplicantsDecreased;
    noOfApplicantsChanged;
    jobDifferenceChanged;
    jobCostNegative: boolean;
    liveEscrowAmount;
    applicantDelta;
    prevFilterCost;
    prevNumberOfApplicants;
    amountPayable;
        ...
        if (this.job.id) {
            ...
            if (this.job.paymentType === PaymentType.AS_YOU_GO) {
                this.amountOutstanding =
                    this.prevOriginalJobCost - this.job.noOfApplicantsBought * this.prevOriginalJobCost / this.job.noOfApplicants;
            }
        } else {
            this.isNew = true;
    ...
    updateGraduationMultiDateCosts() {
        ...
        if (!this.endDateLesser) {
            if (this.graduationFromDate && this.graduationToDate && !(this.graduationToDateAdded && this.graduationFromDateAdded)) {
                ...
                !(this.graduationFromDate && this.graduationToDate) &&
    ...
    save(saveType) {
        if (this.jobEquals(this.job, this.prevJob) && saveType === JobConstants.DRAFT) {
            ...
            if (this.job.id !== undefined) {
                ...
                this.subscribeToSaveResponse(this.jobService.update(this.job), saveType);
            } else {
    ...
    private jobEquals(currentJob, previousJob) {
        ...
        if (
            equal(JSON.stringify(currentJob.jobTitle), JSON.stringify(previousJob.jobTitle)) &&
    ...
    private subscribeToSaveResponse(result: Observable<HttpResponse<Job>>, saveType) {
        result.subscribe(
            (res: HttpResponse<Job>) => this.onSaveSuccess(res.body, saveType),
...
export class JobPopupComponent implements OnInit, OnDestroy {
    ...
    constructor(private route: ActivatedRoute, private jobPopupService: JobPopupService, private dataStorageService: DataStorageService) {}
        ...
        this.routeSub = this.route.params.subscribe(params => {
            if (params['id']) {
                ...
                if (id) {
                    ...
                    this.jobPopupService.open(JobDialogComponent as Component);
                }
            }
        });
    }
