...
export class LanguageService {
    ...
    create(language: Language): Observable<EntityResponseType> {
        ...
        return this.http
            .post<Language>(this.resourceUrl, copy, { observe: 'response' })
    ...
    update(language: Language): Observable<EntityResponseType> {
        const copy = this.convert(language);
        return this.http
            .put<Language>(this.resourceUrl, copy, { observe: 'response' })
            .pipe(map((res: EntityResponseType) => this.convertResponse(res)));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http
            .get<Language>(`${this.resourceUrl}/${id}`, { observe: 'response' })
            .pipe(map((res: EntityResponseType) => this.convertResponse(res)));
    }
    ...
    query(req?: any): Observable<HttpResponse<Language[]>> {
        ...
        return this.http
            .get<Language[]>(this.resourceUrl, { params: options, observe: 'response' })
            .pipe(map((res: HttpResponse<Language[]>) => this.convertArrayResponse(res)));
    }
    ...
    search(req?: any): Observable<HttpResponse<Language[]>> {
        const options = createRequestOption(req);
        return this.http
            .get<Language[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .pipe(map((res: HttpResponse<Language[]>) => this.convertArrayResponse(res)));
    }
