...
export class MaritalStatusService {
    ...
    create(maritalStatus: MaritalStatus): Observable<EntityResponseType> {
        ...
        return this.http.post<MaritalStatus>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    update(maritalStatus: MaritalStatus): Observable<EntityResponseType> {
        const copy = this.convert(maritalStatus);
        return this.http.put<MaritalStatus>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<MaritalStatus>(`${this.resourceUrl}/${id}`, { observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    query(req?: any): Observable<HttpResponse<MaritalStatus[]>> {
        const options = createRequestOption(req);
        return this.http.get<MaritalStatus[]>(this.resourceUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<MaritalStatus[]>) => this.convertArrayResponse(res));
    }
    ...
    search(req?: any): Observable<HttpResponse<MaritalStatus[]>> {
        const options = createRequestOption(req);
        return this.http.get<MaritalStatus[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<MaritalStatus[]>) => this.convertArrayResponse(res));
    }
