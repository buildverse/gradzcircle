...
export class JobFilterService {
    ...
    create(jobFilter: JobFilter): Observable<EntityResponseType> {
        ...
        return this.http.post<JobFilter>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    update(jobFilter: JobFilter): Observable<EntityResponseType> {
        const copy = this.convert(jobFilter);
        return this.http.put<JobFilter>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<JobFilter>(`${this.resourceUrl}/${id}`, { observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    query(req?: any): Observable<HttpResponse<JobFilter[]>> {
        const options = createRequestOption(req);
        return this.http.get<JobFilter[]>(this.resourceUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<JobFilter[]>) => this.convertArrayResponse(res));
    }
    ...
    search(req?: any): Observable<HttpResponse<JobFilter[]>> {
        const options = createRequestOption(req);
        return this.http.get<JobFilter[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<JobFilter[]>) => this.convertArrayResponse(res));
    }
