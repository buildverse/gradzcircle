...
export class EmployabilityService {
    ...
    create(employability: Employability): Observable<EntityResponseType> {
        ...
        return this.http.post<Employability>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    update(employability: Employability): Observable<EntityResponseType> {
        const copy = this.convert(employability);
        return this.http.put<Employability>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<Employability>(`${this.resourceUrl}/${id}`, { observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    query(req?: any): Observable<HttpResponse<Employability[]>> {
        const options = createRequestOption(req);
        return this.http.get<Employability[]>(this.resourceUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<Employability[]>) => this.convertArrayResponse(res));
    }
    ...
    search(req?: any): Observable<HttpResponse<Employability[]>> {
        const options = createRequestOption(req);
        return this.http.get<Employability[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<Employability[]>) => this.convertArrayResponse(res));
    }
