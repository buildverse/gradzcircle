...
export class CaptureUniversityService {
    ...
    create(captureUniversity: CaptureUniversity): Observable<EntityResponseType> {
        ...
        return this.http.post<CaptureUniversity>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    update(captureUniversity: CaptureUniversity): Observable<EntityResponseType> {
        const copy = this.convert(captureUniversity);
        return this.http.put<CaptureUniversity>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<CaptureUniversity>(`${this.resourceUrl}/${id}`, { observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    query(req?: any): Observable<HttpResponse<CaptureUniversity[]>> {
        const options = createRequestOption(req);
        return this.http.get<CaptureUniversity[]>(this.resourceUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<CaptureUniversity[]>) => this.convertArrayResponse(res));
    }
    ...
    search(req?: any): Observable<HttpResponse<CaptureUniversity[]>> {
        const options = createRequestOption(req);
        return this.http.get<CaptureUniversity[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<CaptureUniversity[]>) => this.convertArrayResponse(res));
    }
