...
export class AppConfigService {
    ...
    create(appConfig: AppConfig): Observable<EntityResponseType> {
        ...
        return this.http.post<AppConfig>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    update(appConfig: AppConfig): Observable<EntityResponseType> {
        const copy = this.convert(appConfig);
        return this.http.put<AppConfig>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<AppConfig>(`${this.resourceUrl}/${id}`, { observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    query(req?: any): Observable<HttpResponse<AppConfig[]>> {
        const options = createRequestOption(req);
        return this.http.get<AppConfig[]>(this.resourceUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<AppConfig[]>) => this.convertArrayResponse(res));
    }
    ...
    search(req?: any): Observable<HttpResponse<AppConfig[]>> {
        const options = createRequestOption(req);
        return this.http.get<AppConfig[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<AppConfig[]>) => this.convertArrayResponse(res));
    }
