...
export class CorporateDialogComponent implements OnInit {
    ...
    save() {
        ...
        if (this.corporate.id !== undefined) {
            this.subscribeToSaveResponse(this.corporateService.update(this.corporate));
        } else {
            this.subscribeToSaveResponse(this.corporateService.create(this.corporate));
        }
    }
    ...
    private subscribeToSaveResponse(result: Observable<HttpResponse<Corporate>>) {
        result.subscribe((res: HttpResponse<Corporate>) => this.onSaveSuccess(res.body), (res: HttpErrorResponse) => this.onSaveError());
    }
...
export class CorporatePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.routeSub = this.route.params.subscribe(params => {
            if (params['id']) {
                this.corporatePopupService.open(CorporateDialogComponent as Component, params['id']);
            } else {
                const id = this.dataService.getData(CORPORATE_ID);
                if (id) {
                    this.corporatePopupService.open(CorporateDialogComponent as Component, id);
                } else {
                    this.corporatePopupService.open(CorporateDialogComponent as Component);
                }
            }
        });
    }
