...
export class CandidateService {
    ...
    create(candidate: Candidate): Observable<EntityResponseType> {
        ...
        return this.http
            .post<Candidate>(this.resourceUrl, copy, { observe: 'response' })
    ...
    update(candidate: Candidate): Observable<EntityResponseType> {
        const copy = this.convert(candidate);
        return this.http
            .put<Candidate>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    linkCandidateAndCorporateForJob(candidateId: any, jobId: number, corporateId: number): Observable<EntityResponseType> {
        return this.http
            .get<Candidate>(`${this.resourceCandidateToCorporateLink}/${candidateId}/${jobId}/${corporateId}`, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    getCandidateByLoginId(id: string): Observable<EntityResponseType> {
        return this.http
            .get<Candidate>(`api/candidateByLogin/${id}`, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    getCandidateByCandidateId(id: string): Observable<EntityResponseType> {
        return this.http
            .get<Candidate>(`${this.resourceUrlGetCandidateById}/${id}`, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    getCandidateDetails(id: string): Observable<EntityResponseType> {
        return this.http
            .get<Candidate>(`${this.resourceUrlGetCandidateDetails}/${id}`, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }
    ...
    getCandidatePublicProfile(candidateId: any, jobId: number, corporateId: number): Observable<EntityResponseTypeCandidatePublicProfile> {
        return this.http
            .get<CandidatePublicProfile>(`${this.resourceCandidatePublicProfile}/${candidateId}/${jobId}/${corporateId}`, {
                observe: 'response'
            })
    ...
    find(id: number): Observable<EntityResponseType> {
        return this.http
            .get<Candidate>(`${this.resourceUrl}/${id}`, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }
    ...
    query(req?: any): Observable<HttpResponse<Candidate[]>> {
        ...
        return this.http
            .get<Candidate[]>(this.resourceUrl, { params: options, observe: 'response' })
    ...
    queryForGuest(req?: any): Observable<HttpResponse<CandidateList[]>> {
        const options = createRequestOption(req);
        return this.http
            .get<CandidateList[]>(this.resourceUrlGuest, { params: options, observe: 'response' })
    ...
    search(req?: any): Observable<HttpResponse<Candidate[]>> {
        const options = createRequestOption(req);
        return this.http
            .get<Candidate[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<Candidate[]>) => this.convertArrayResponse(res));
    }
    ...
    private convertItemFromServer(candidate: Candidate): Candidate {
        ...
        copy.dateOfBirth = this.dateUtils.convertLocalDateFromServer(candidate.dateOfBirth);
    ...
    private convert(candidate: Candidate): Candidate {
        const copy: Candidate = Object.assign({}, candidate);
        copy.dateOfBirth = this.dateUtils.convertLocalDateToServer(candidate.dateOfBirth);
