...
export class CandidateEmploymentComponent implements OnInit, OnDestroy {
  ...
  registerChangeInCandidateEmployments() {
    if (this.candidateId) {
      this.employemntEventSubscriber = this.eventManager.subscribe('candidateEmploymentListModification', (response) => this.loadEmploymentsForCandidate());
      this.projectEventSubscriber = this.eventManager.subscribe('candidateProjectListModification', (response) => this.loadEmploymentsForCandidate());
    } else {
