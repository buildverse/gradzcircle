...
export class CaptureCollegeService {
    ...
    create(captureCollege: CaptureCollege): Observable<EntityResponseType> {
        ...
        return this.http.post<CaptureCollege>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    update(captureCollege: CaptureCollege): Observable<EntityResponseType> {
        const copy = this.convert(captureCollege);
        return this.http.put<CaptureCollege>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<CaptureCollege>(`${this.resourceUrl}/${id}`, { observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    query(req?: any): Observable<HttpResponse<CaptureCollege[]>> {
        const options = createRequestOption(req);
        return this.http.get<CaptureCollege[]>(this.resourceUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<CaptureCollege[]>) => this.convertArrayResponse(res));
    }
    ...
    search(req?: any): Observable<HttpResponse<CaptureCollege[]>> {
        const options = createRequestOption(req);
        return this.http.get<CaptureCollege[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<CaptureCollege[]>) => this.convertArrayResponse(res));
    }
