    ...
    ngOnInit() {
       ...
       if (this.principal.hasAnyAuthorityDirect(['ROLE_ADMIN'])) {
        ...
        this.skillsService.query()
            .subscribe((res: HttpResponse<Skills[]>) => { this.skills = res.body; }, (res: HttpErrorResponse) => this.onError(res.message));
    ...
    save() {
        ...
        if (this.candidateSkills.id !== undefined) {
            this.subscribeToSaveResponse(
                this.candidateSkillsService.update(this.candidateSkills));
        } else {
            this.subscribeToSaveResponse(
                this.candidateSkillsService.create(this.candidateSkills));
        }
    }

    private subscribeToSaveResponse(result: Observable<HttpResponse<CandidateSkills>>) {
        result.subscribe((res: HttpResponse<CandidateSkills>) =>
        ...
        this.routeSub = this.route.params.subscribe((params) => {
            if ( params['id'] ) {
                this.candidateSkillsPopupService
                    .open(CandidateSkillsDialogComponent as Component, params['id']);
            } else {
                this.candidateSkillsPopupService
                    .open(CandidateSkillsDialogComponent as Component);
            }
        });
    }
    ...
    ngOnInit() {
        this.routeSub = this.route.params.subscribe((params) => {
            if ( params['id'] ) {
                ...
                this.candidateSkillsPopupService
                    .open(CandidateSkillsDialogComponent as Component, id);
            }
        });
    }
