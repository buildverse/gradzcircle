...
export class CandidateSkillsService {
  ...
  create(candidateSkills: CandidateSkills): Observable<EntityResponseType> {
    ...
    return this.http.post<CandidateSkills>(this.resourceUrl, copy, {observe: 'response'})
      .map((res: EntityResponseType) => this.convertResponse(res));
  }

  update(candidateSkills: CandidateSkills): Observable<EntityResponseType> {
    const copy = this.convert(candidateSkills);
    return this.http.put<CandidateSkills>(this.resourceUrl, copy, {observe: 'response'})
      .map((res: EntityResponseType) => this.convertResponse(res));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<CandidateSkills>(`${this.resourceUrl}/${id}`, {observe: 'response'})
      .map((res: EntityResponseType) => this.convertResponse(res));
  }

  query(req?: any): Observable<HttpResponse<CandidateSkills[]>> {
    const options = createRequestOption(req);
    return this.http.get<CandidateSkills[]>(this.resourceUrl, {params: options, observe: 'response'})
      .map((res: HttpResponse<CandidateSkills[]>) => this.convertArrayResponse(res));
  }
  ...
  search(req?: any): Observable<HttpResponse<CandidateSkills[]>> {
    const options = createRequestOption(req);
    return this.http.get<CandidateSkills[]>(this.resourceSearchUrl, {params: options, observe: 'response'})
      .map((res: HttpResponse<CandidateSkills[]>) => this.convertArrayResponse(res));
  }

  findSkillsByCandidateId(id: number): Observable<HttpResponse<CandidateSkills[]>> {
    return this.http.get<CandidateSkills[]>(`${this.resourceGetSkillsForCandidateUrl}/${id}`, {observe: 'response'})
      .map((res: HttpResponse<CandidateSkills[]>) => this.convertArrayResponse(res));
  }
