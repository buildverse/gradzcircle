...
export class AuditDialogComponent implements OnInit {
    ...
    constructor(public activeModal: NgbActiveModal, private auditService: AuditService, private eventManager: JhiEventManager) {}
    ...
    private subscribeToSaveResponse(result: Observable<HttpResponse<Audit>>) {
        result.subscribe((res: HttpResponse<Audit>) => this.onSaveSuccess(res.body), (res: HttpErrorResponse) => this.onSaveError());
    }
...
export class AuditPopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.routeSub = this.route.params.subscribe(params => {
            if (params['id']) {
                this.auditPopupService.open(AuditDialogComponent as Component, params['id']);
            } else {
                this.auditPopupService.open(AuditDialogComponent as Component);
            }
        });
    }
