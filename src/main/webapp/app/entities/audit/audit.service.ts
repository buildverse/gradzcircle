...
export class AuditService {
    ...
    create(audit: Audit): Observable<EntityResponseType> {
        ...
        return this.http
            .post<Audit>(this.resourceUrl, copy, { observe: 'response' })
    ...
    update(audit: Audit): Observable<EntityResponseType> {
        const copy = this.convert(audit);
        return this.http
            .put<Audit>(this.resourceUrl, copy, { observe: 'response' })
            .pipe(map((res: EntityResponseType) => this.convertResponse(res)));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http
            .get<Audit>(`${this.resourceUrl}/${id}`, { observe: 'response' })
            .pipe(map((res: EntityResponseType) => this.convertResponse(res)));
    }
    ...
    query(req?: any): Observable<HttpResponse<Audit[]>> {
        ...
        return this.http
            .get<Audit[]>(this.resourceUrl, { params: options, observe: 'response' })
            .pipe(map((res: HttpResponse<Audit[]>) => this.convertArrayResponse(res)));
    }
    ...
    search(req?: any): Observable<HttpResponse<Audit[]>> {
        const options = createRequestOption(req);
        return this.http
            .get<Audit[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .pipe(map((res: HttpResponse<Audit[]>) => this.convertArrayResponse(res)));
    }
    ...
    private convertItemFromServer(audit: Audit): Audit {
        ...
        copy.createdTime = this.dateUtils.convertDateTimeFromServer(audit.createdTime);
        copy.updatedTime = this.dateUtils.convertDateTimeFromServer(audit.updatedTime);
        return copy;
    }
