...
public class CaptureCourseResource {
	...
	public CaptureCourseResource(CaptureCourseRepository captureCourseRepository,
	 ...
	 *
	 * @param captureCourse
	 *            the captureCourse to create
	 * @return the ResponseEntity with status 201 (Created) and with body the new
	 *         captureCourse, or with status 400 (Bad Request) if the captureCourse
	 *         has already an ID
	...
	@Timed
	public ResponseEntity<CaptureCourse> createCaptureCourse(@RequestBody CaptureCourse captureCourse)
			throws URISyntaxException {
		log.debug("REST request to save CaptureCourse : {}", captureCourse);
		if (captureCourse.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists",
					"A new captureCourse cannot already have an ID")).body(null);
		}
	 ...
	 *
	 * @param captureCourse
	 *            the captureCourse to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         captureCourse, or with status 400 (Bad Request) if the captureCourse
	 *         is not valid, or with status 500 (Internal Server Error) if the
	 *         captureCourse couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PutMapping("/capture-courses")
	@Timed
	public ResponseEntity<CaptureCourse> updateCaptureCourse(@RequestBody CaptureCourse captureCourse)
			throws URISyntaxException {
	 ...
	 *
	 * @param id
	 *            the id of the captureCourse to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the
	 *         captureCourse, or with status 404 (Not Found)
	 */
	 ...
	 *
	 * @param id
	 *            the id of the captureCourse to delete
	...
	/**
	 * SEARCH /_search/capture-courses?query=:query : search for the captureCourse
	 * corresponding to the query.
	 *
	 * @param query
	 *            the query of the captureCourse search
	...
	public List<CaptureCourse> searchCaptureCourses(@RequestParam String query) {
		...
		return StreamSupport.stream(captureCourseSearchRepository.search(queryStringQuery(query)).spliterator(), false)
