...
public class JobTypeResource {
	 ...
	 *
	 * @param jobType
	 *            the jobType to create
	 * @return the ResponseEntity with status 201 (Created) and with body the new
	 *         jobType, or with status 400 (Bad Request) if the jobType has already
	 *         an ID
	...
	public ResponseEntity<JobType> createJobType(@RequestBody JobType jobType) throws URISyntaxException {
		...
		if (jobType.getId() != null) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new jobType cannot already have an ID"))
					.body(null);
		}
	 ...
	 *         jobType, or with status 400 (Bad Request) if the jobType is not
	 *         valid, or with status 500 (Internal Server Error) if the jobType
	 *         couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<JobType> updateJobType(@RequestBody JobType jobType) throws URISyntaxException {
		...
		JobType result = jobTypeRepository.save(jobType);
		jobTypeSearchRepository.save(result);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, jobType.getId().toString()))
				.body(result);
	}
	 ...
	 *
	 * @return the ResponseEntity with status 200 (OK) and the list of jobTypes in
	 *         body
	 */
	 ...
	 *
	 * @param id
	 *            the id of the jobType to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the jobType, or
	 ...
	 *
	 * @param id
	 *            the id of the jobType to delete
	 ...
	 *
	 * @param query
	 *            the query of the jobType search
	...
	public List<JobType> searchJobTypes(@RequestParam String query) {
		...
		return StreamSupport.stream(jobTypeSearchRepository.search(queryStringQuery(query)).spliterator(), false)
