...
public class SkillsResource {
	 ...
	 *         skills, or with status 400 (Bad Request) if the skills has already an
	 *         ID
	...
	public ResponseEntity<Skills> createSkills(@RequestBody Skills skills) throws URISyntaxException {
		...
		if (skills.getId() != null) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new skills cannot already have an ID"))
					.body(null);
		}
	 ...
	 *         skills, or with status 400 (Bad Request) if the skills is not valid,
	 *         or with status 500 (Internal Server Error) if the skills couldn't be
	 *         updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<Skills> updateSkills(@RequestBody Skills skills) throws URISyntaxException {
		...
		Skills result = skillsRepository.save(skills);
		skillsSearchRepository.save(result);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, skills.getId().toString()))
				.body(result);
	}
	 ...
	 *
	 * @return the ResponseEntity with status 200 (OK) and the list of skills in
	 *         body
	 */
	@GetMapping("/skills")
	@Timed
	 ...
	 *
	 * @param id
	 *            the id of the skills to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the skills, or
	 ...
	 *
	 * @param id
	 *            the id of the skills to delete
	...
	/**
	 * SEARCH /_search/skills?query=:query : search for the skills corresponding to
	 * the query.
	 *
	 * @param query
	 *            the query of the skills search
	...
	public List<Skills> searchSkills(@RequestParam String query) {
		...
		return StreamSupport.stream(skillsSearchRepository.search(queryStringQuery(query)).spliterator(), false)
