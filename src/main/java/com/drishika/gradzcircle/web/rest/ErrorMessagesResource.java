...
public class ErrorMessagesResource {
    ...
    public ErrorMessagesResource(ErrorMessagesRepository errorMessagesRepository, ErrorMessagesSearchRepository errorMessagesSearchRepository) {
     ...
     *
     * @param errorMessages the errorMessages to create
     * @return the ResponseEntity with status 201 (Created) and with body the new errorMessages, or with status 400 (Bad Request) if the errorMessages has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/error-messages")
    @Timed
    public ResponseEntity<ErrorMessages> createErrorMessages(@RequestBody ErrorMessages errorMessages) throws URISyntaxException {
        log.debug("REST request to save ErrorMessages : {}", errorMessages);
        if (errorMessages.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new errorMessages cannot already have an ID")).body(null);
        }
     ...
     *
     * @param errorMessages the errorMessages to update
     * @return the ResponseEntity with status 200 (OK) and with body the updated errorMessages,
     * or with status 400 (Bad Request) if the errorMessages is not valid,
     * or with status 500 (Internal Server Error) if the errorMessages couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
     ...
     *
     * @return the ResponseEntity with status 200 (OK) and the list of errorMessages in body
     */
     ...
     *
     * @param id the id of the errorMessages to retrieve
     * @return the ResponseEntity with status 200 (OK) and with body the errorMessages, or with status 404 (Not Found)
     */
     ...
     *
     * @param id the id of the errorMessages to delete
    ...
    /**
     * SEARCH  /_search/error-messages?query=:query : search for the errorMessages corresponding
     * to the query.
     *
     * @param query the query of the errorMessages search
    ...
    public List<ErrorMessages> searchErrorMessages(@RequestParam String query) {
        ...
        return StreamSupport
            .stream(errorMessagesSearchRepository.search(queryStringQuery(query)).spliterator(), false)
