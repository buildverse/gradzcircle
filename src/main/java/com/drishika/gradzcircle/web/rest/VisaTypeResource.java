...
public class VisaTypeResource {
	 ...
	 *
	 * @param visaType
	 *            the visaType to create
	 * @return the ResponseEntity with status 201 (Created) and with body the new
	 *         visaType, or with status 400 (Bad Request) if the visaType has
	 *         already an ID
	...
	public ResponseEntity<VisaType> createVisaType(@RequestBody VisaType visaType) throws URISyntaxException {
		...
		if (visaType.getId() != null) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new visaType cannot already have an ID"))
					.body(null);
		}
	 ...
	 *
	 * @param visaType
	 *            the visaType to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         visaType, or with status 400 (Bad Request) if the visaType is not
	 *         valid, or with status 500 (Internal Server Error) if the visaType
	 *         couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<VisaType> updateVisaType(@RequestBody VisaType visaType) throws URISyntaxException {
		...
		VisaType result = visaTypeRepository.save(visaType);
		visaTypeSearchRepository.save(result);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, visaType.getId().toString()))
				.body(result);
	}
	 ...
	 *
	 * @return the ResponseEntity with status 200 (OK) and the list of visaTypes in
	 *         body
	 */
	 ...
	 *
	 * @param id
	 *            the id of the visaType to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the visaType,
	 ...
	 *
	 * @param id
	 *            the id of the visaType to delete
	...
	/**
	 * SEARCH /_search/visa-types?query=:query : search for the visaType
	 * corresponding to the query.
	 *
	 * @param query
	 *            the query of the visaType search
	...
	public List<VisaType> searchVisaTypes(@RequestParam String query) {
		...
		return StreamSupport.stream(visaTypeSearchRepository.search(queryStringQuery(query)).spliterator(), false)
