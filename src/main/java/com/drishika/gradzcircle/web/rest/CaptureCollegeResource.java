...
public class CaptureCollegeResource {
	...
	public CaptureCollegeResource(CaptureCollegeRepository captureCollegeRepository,
	 ...
	 *
	 * @param captureCollege
	 *            the captureCollege to create
	 * @return the ResponseEntity with status 201 (Created) and with body the new
	 *         captureCollege, or with status 400 (Bad Request) if the
	 *         captureCollege has already an ID
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PostMapping("/capture-colleges")
	@Timed
	public ResponseEntity<CaptureCollege> createCaptureCollege(@RequestBody CaptureCollege captureCollege)
			throws URISyntaxException {
		log.debug("REST request to save CaptureCollege : {}", captureCollege);
		if (captureCollege.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists",
					"A new captureCollege cannot already have an ID")).body(null);
		}
	 ...
	 *
	 * @param captureCollege
	 *            the captureCollege to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         captureCollege, or with status 400 (Bad Request) if the
	 *         captureCollege is not valid, or with status 500 (Internal Server
	 *         Error) if the captureCollege couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PutMapping("/capture-colleges")
	@Timed
	public ResponseEntity<CaptureCollege> updateCaptureCollege(@RequestBody CaptureCollege captureCollege)
			throws URISyntaxException {
	 ...
	 *
	 * @param id
	 *            the id of the captureCollege to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the
	 *         captureCollege, or with status 404 (Not Found)
	 */
	 ...
	 *
	 * @param id
	 *            the id of the captureCollege to delete
	...
	/**
	 * SEARCH /_search/capture-colleges?query=:query : search for the captureCollege
	 * corresponding to the query.
	 *
	 * @param query
	 *            the query of the captureCollege search
	...
	public List<CaptureCollege> searchCaptureColleges(@RequestParam String query) {
		...
		return StreamSupport.stream(captureCollegeSearchRepository.search(queryStringQuery(query)).spliterator(), false)
