...
public class JobCategoryResource {
	...
	public JobCategoryResource(JobCategoryRepository jobCategoryRepository,
	 ...
	 *
	 * @param jobCategory
	 *            the jobCategory to create
	 * @return the ResponseEntity with status 201 (Created) and with body the new
	 *         jobCategory, or with status 400 (Bad Request) if the jobCategory has
	 *         already an ID
	...
	public ResponseEntity<JobCategory> createJobCategory(@RequestBody JobCategory jobCategory)
		...
		if (jobCategory.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists",
					"A new jobCategory cannot already have an ID")).body(null);
		}
	 ...
	 *
	 * @param jobCategory
	 *            the jobCategory to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         jobCategory, or with status 400 (Bad Request) if the jobCategory is
	 *         not valid, or with status 500 (Internal Server Error) if the
	 *         jobCategory couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<JobCategory> updateJobCategory(@RequestBody JobCategory jobCategory)
		...
		return ResponseEntity.ok()
				.headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, jobCategory.getId().toString())).body(result);
	}

	/**
	 * GET /job-categories : get all the jobCategories.
	 *
	 * @return the ResponseEntity with status 200 (OK) and the list of jobCategories
	 *         in body
	 */
	 ...
	 *
	 * @param id
	 *            the id of the jobCategory to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the
	 *         jobCategory, or with status 404 (Not Found)
	 */
	 ...
	 *
	 * @param id
	 *            the id of the jobCategory to delete
	...
	/**
	 * SEARCH /_search/job-categories?query=:query : search for the jobCategory
	 * corresponding to the query.
	 *
	 * @param query
	 *            the query of the jobCategory search
	...
	public List<JobCategory> searchJobCategories(@RequestParam String query) {
		...
		return StreamSupport.stream(jobCategorySearchRepository.search(queryStringQuery(query)).spliterator(), false)
