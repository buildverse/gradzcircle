...
public class FilterResource {
	 ...
	 *         filter, or with status 400 (Bad Request) if the filter has already an
	 *         ID
	...
	public ResponseEntity<Filter> createFilter(@RequestBody Filter filter) throws URISyntaxException {
		...
		if (filter.getId() != null) {
			return ResponseEntity.badRequest().headers(
					HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new filter cannot already have an ID"))
					.body(null);
		}
	 ...
	 *         filter, or with status 400 (Bad Request) if the filter is not valid,
	 *         or with status 500 (Internal Server Error) if the filter couldn't be
	 *         updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<Filter> updateFilter(@RequestBody Filter filter) throws URISyntaxException {
		...
		Filter result = filterRepository.save(filter);
		filterSearchRepository.save(result);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, filter.getId().toString()))
				.body(result);
	}
	 ...
	 *
	 * @return the ResponseEntity with status 200 (OK) and the list of filters in
	 *         body
	 */
	@GetMapping("/filters")
	@Timed
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the filter, or
	...
	/**
	 * SEARCH /_search/filters?query=:query : search for the filter corresponding to
	 * the query.
	 *
	 * @param query
	 *            the query of the filter search
	...
	public List<Filter> searchFilters(@RequestParam String query) {
		...
		return StreamSupport.stream(filterSearchRepository.search(queryStringQuery(query)).spliterator(), false)
