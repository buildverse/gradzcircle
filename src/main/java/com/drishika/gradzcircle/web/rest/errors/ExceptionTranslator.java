package com.drishika.gradzcircle.web.rest.errors;
...
public class ExceptionTranslator implements ProblemHandling {
	 ...
	 */
	@Override
	public ResponseEntity<Problem> handleThrowable(@Nonnull final Throwable throwable,
		...
		if (responseStatus != null) {
			Problem problem = Problem.builder().withStatus(new HttpStatusAdapter(responseStatus.value()))
					.withTitle(responseStatus.reason().isEmpty() ? responseStatus.value().getReasonPhrase()
							: responseStatus.reason())
					.withDetail(throwable.getMessage()).build();
