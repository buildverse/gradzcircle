...
public class FileSystemStorageService implements StorageService {
	...
	public FileSystemStorageService(ApplicationProperties applicationProperties, UserRepository userRepository,
	...
	public void store(MultipartFile file, Long userId) {
		...
		try {
			...
			try (Stream<Path> stream = Files.walk(start, 1)) {
				joined = stream.map(String::valueOf)
						.filter(path -> (path.substring(path.lastIndexOf("/") + 1, path.length()).split("\\.")[0])
