...
public class JobFilterParserUnitTest {
	...
	@Test
	public void testCreateFilterObjectWithGraduationDate() throws Exception {
	...
	//	System.out.println(jobFilterParser.getFilterObjectFromJson(filterDesc));
		...
		assertThat(jobFilterParser.getFilterObjectFromJson(filterDesc).getPercent()).isEqualTo(null);
		...
		assertThat(jobFilterParser.getFilterObjectFromJson(filterDesc).getGraduationDate().getGraduationDate()).isAfterOrEqualTo(LocalDate.of(2018, 5, 17));
		assertThat(jobFilterParser.getFilterObjectFromJson(filterDesc).getGender().getId()).isEqualTo(gender.getId());
		assertThat(jobFilterParser.getFilterObjectFromJson(filterDesc).getGender().getGender()).isEqualTo(gender.getGender());
		assertThat(jobFilterParser.getFilterObjectFromJson(filterDesc).getLanguages().size()).isEqualTo(2);
	}
    
    @Test
	public void testCreateFilterObjectWithGraduationFromAndToDate() throws Exception {
		String filterDesc = "{\"basic\":true,\"colleges\":[{\"value\":\"Nanyang Academy of Fine Arts\",\"display\":\"Nanyang Academy of Fine Arts\"}],\"premium\":true,\""
				+ "courses\":[{\"value\":\"Computer Engg.\",\"display\":\"Computer Engg.\"}],\"qualifications\":[{\"value\":\"Master\",\"display\":\"Master\"}],"
				+ "\"scoreType\":\"gpa\",\"gpa\":\"7.7\",\"addOn\":true,\"graduationDateType\":\"between\",\"graduationFromDate\":{\"year\":2018,\"month\":6,\"day\":6},"
		...
		assertThat(jobFilterParser.getFilterObjectFromJson(filterDesc).getGraduationFromDate().getGraduationDate()).isAfterOrEqualTo(LocalDate.of(2018, 6, 6));
		assertThat(jobFilterParser.getFilterObjectFromJson(filterDesc).getGraduationToDate().getGraduationDate()).isAfterOrEqualTo(LocalDate.of(2018, 6, 16));
		assertThat(jobFilterParser.getFilterObjectFromJson(filterDesc).getGender().getId()).isEqualTo(gender.getId());
		assertThat(jobFilterParser.getFilterObjectFromJson(filterDesc).getGender().getGender()).isEqualTo(gender.getGender());
	}
}
