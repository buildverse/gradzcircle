...
public class MaritalStatusResourceIntTest {
	...
	public static MaritalStatus createEntity(EntityManager em) {
		MaritalStatus maritalStatus = new MaritalStatus().status(DEFAULT_STATUS);
	...
	public void createMaritalStatus() throws Exception {
		...
		restMaritalStatusMockMvc.perform(post("/api/marital-statuses").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(maritalStatus))).andExpect(status().isCreated());
	...
	public void createMaritalStatusWithExistingId() throws Exception {
		...
		restMaritalStatusMockMvc.perform(post("/api/marital-statuses").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(maritalStatus))).andExpect(status().isBadRequest());
	...
	public void getAllMaritalStatuses() throws Exception {
		...
		restMaritalStatusMockMvc.perform(get("/api/marital-statuses?sort=id,desc")).andExpect(status().isOk())
	...
	public void updateMaritalStatus() throws Exception {
		...
		updatedMaritalStatus.status(UPDATED_STATUS);

		restMaritalStatusMockMvc.perform(put("/api/marital-statuses").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(updatedMaritalStatus))).andExpect(status().isOk());
	...
	public void deleteMaritalStatus() throws Exception {
		...
		// Get the maritalStatus
		restMaritalStatusMockMvc.perform(
				delete("/api/marital-statuses/{id}", maritalStatus.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
