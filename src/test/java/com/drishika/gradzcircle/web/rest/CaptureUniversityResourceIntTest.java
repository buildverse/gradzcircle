...
public class CaptureUniversityResourceIntTest {
	...
	public void setup() {
		...
		final CaptureUniversityResource captureUniversityResource = new CaptureUniversityResource(
				captureUniversityRepository, captureUniversitySearchRepository);
	...
	public static CaptureUniversity createEntity(EntityManager em) {
		CaptureUniversity captureUniversity = new CaptureUniversity().universityName(DEFAULT_UNIVERSITY_NAME);
	...
	public void createCaptureUniversity() throws Exception {
		...
		restCaptureUniversityMockMvc
				.perform(post("/api/capture-universities").contentType(TestUtil.APPLICATION_JSON_UTF8)
		...
		CaptureUniversity captureUniversityEs = captureUniversitySearchRepository
				.findOne(testCaptureUniversity.getId());
		assertThat(captureUniversityEs).isEqualToComparingFieldByField(testCaptureUniversity);
	}
	...
	public void getAllCaptureUniversities() throws Exception {
		...
		restCaptureUniversityMockMvc.perform(get("/api/capture-universities?sort=id,desc")).andExpect(status().isOk())
	...
	public void updateCaptureUniversity() throws Exception {
		...
		updatedCaptureUniversity.universityName(UPDATED_UNIVERSITY_NAME);

		restCaptureUniversityMockMvc
				.perform(put("/api/capture-universities").contentType(TestUtil.APPLICATION_JSON_UTF8)
	...
	public void searchCaptureUniversity() throws Exception {
		...
		restCaptureUniversityMockMvc
				.perform(get("/api/_search/capture-universities?query=id:" + captureUniversity.getId()))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.[*].id").value(hasItem(captureUniversity.getId().intValue())))
				.andExpect(jsonPath("$.[*].universityName").value(hasItem(DEFAULT_UNIVERSITY_NAME.toString())));
	}
