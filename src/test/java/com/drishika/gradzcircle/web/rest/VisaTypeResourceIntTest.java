...
public class VisaTypeResourceIntTest {
	...
	public static VisaType createEntity(EntityManager em) {
		VisaType visaType = new VisaType().visa(DEFAULT_VISA);
		return visaType;
	}
	...
	public void createVisaType() throws Exception {
		...
		// Create the VisaType
		restVisaTypeMockMvc.perform(post("/api/visa-types").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(visaType))).andExpect(status().isCreated());
		...
		assertThat(visaTypeEs).isEqualToComparingFieldByField(testVisaType);
	}
	...
	public void createVisaTypeWithExistingId() throws Exception {
		...
		restVisaTypeMockMvc.perform(post("/api/visa-types").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(visaType))).andExpect(status().isBadRequest());
	...
	public void getAllVisaTypes() throws Exception {
		...
		restVisaTypeMockMvc.perform(get("/api/visa-types?sort=id,desc")).andExpect(status().isOk())
	...
	public void getVisaType() throws Exception {
		...
		// Get the visaType
		restVisaTypeMockMvc.perform(get("/api/visa-types/{id}", visaType.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void getNonExistingVisaType() throws Exception {
		// Get the visaType
		restVisaTypeMockMvc.perform(get("/api/visa-types/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
	}
	...
	public void updateVisaType() throws Exception {
		...
		updatedVisaType.visa(UPDATED_VISA);

		restVisaTypeMockMvc.perform(put("/api/visa-types").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(updatedVisaType))).andExpect(status().isOk());
	...
	public void deleteVisaType() throws Exception {
		...
		// Get the visaType
		restVisaTypeMockMvc
				.perform(delete("/api/visa-types/{id}", visaType.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
