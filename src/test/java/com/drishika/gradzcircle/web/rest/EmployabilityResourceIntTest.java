...
public class EmployabilityResourceIntTest {
	...
	public void setup() {
		...
		final EmployabilityResource employabilityResource = new EmployabilityResource(employabilityRepository,
	...
	public static Employability createEntity(EntityManager em) {
		Employability employability = new Employability().employableSkillName(DEFAULT_EMPLOYABLE_SKILL_NAME)
	...
	public void createEmployability() throws Exception {
		...
		restEmployabilityMockMvc.perform(post("/api/employabilities").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(employability))).andExpect(status().isCreated());
		...
		assertThat(employabilityEs).isEqualToComparingFieldByField(testEmployability);
	}
	...
	public void createEmployabilityWithExistingId() throws Exception {
		...
		restEmployabilityMockMvc.perform(post("/api/employabilities").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(employability))).andExpect(status().isBadRequest());
	...
	public void getAllEmployabilities() throws Exception {
		...
		restEmployabilityMockMvc.perform(get("/api/employabilities?sort=id,desc")).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.[*].id").value(hasItem(employability.getId().intValue())))
				.andExpect(
						jsonPath("$.[*].employableSkillName").value(hasItem(DEFAULT_EMPLOYABLE_SKILL_NAME.toString())))
	...
	public void updateEmployability() throws Exception {
		...
		updatedEmployability.employableSkillName(UPDATED_EMPLOYABLE_SKILL_NAME)
				.employabilityScore(UPDATED_EMPLOYABILITY_SCORE)
				.employabilityPercentile(UPDATED_EMPLOYABILITY_PERCENTILE);

		restEmployabilityMockMvc.perform(put("/api/employabilities").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(updatedEmployability))).andExpect(status().isOk());
	...
	public void deleteEmployability() throws Exception {
		...
		// Get the employability
		restEmployabilityMockMvc.perform(
				delete("/api/employabilities/{id}", employability.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
