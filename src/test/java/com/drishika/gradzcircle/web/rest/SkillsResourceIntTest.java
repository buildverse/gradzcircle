...
public class SkillsResourceIntTest {
	...
	public static Skills createEntity(EntityManager em) {
		Skills skills = new Skills().skill(DEFAULT_SKILL);
		return skills;
	}
	...
	public void createSkills() throws Exception {
		...
		// Create the Skills
		restSkillsMockMvc.perform(post("/api/skills").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(skills))).andExpect(status().isCreated());
	...
	public void createSkillsWithExistingId() throws Exception {
		...
		restSkillsMockMvc.perform(post("/api/skills").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(skills))).andExpect(status().isBadRequest());
	...
	public void getAllSkills() throws Exception {
		...
		restSkillsMockMvc.perform(get("/api/skills?sort=id,desc")).andExpect(status().isOk())
	...
	public void getSkills() throws Exception {
		...
		// Get the skills
		restSkillsMockMvc.perform(get("/api/skills/{id}", skills.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void getNonExistingSkills() throws Exception {
		// Get the skills
		restSkillsMockMvc.perform(get("/api/skills/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
	}
	...
	public void updateSkills() throws Exception {
		...
		updatedSkills.skill(UPDATED_SKILL);

		restSkillsMockMvc.perform(put("/api/skills").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(updatedSkills))).andExpect(status().isOk());
	...
	public void deleteSkills() throws Exception {
		...
		// Get the skills
		restSkillsMockMvc.perform(delete("/api/skills/{id}", skills.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
	...
	public void searchSkills() throws Exception {
		...
		// Search the skills
		restSkillsMockMvc.perform(get("/api/_search/skills?query=id:" + skills.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.[*].id").value(hasItem(skills.getId().intValue())))
				.andExpect(jsonPath("$.[*].skill").value(hasItem(DEFAULT_SKILL.toString())));
	}
