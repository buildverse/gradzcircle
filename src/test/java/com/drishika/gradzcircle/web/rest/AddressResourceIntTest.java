...
public class AddressResourceIntTest {
	...
	public static Address createEntity(EntityManager em) {
		Address address = new Address().addressLineOne(DEFAULT_ADDRESS_LINE_ONE)
				.addressLineTwo(DEFAULT_ADDRESS_LINE_TWO).city(DEFAULT_CITY).state(DEFAULT_STATE).zip(DEFAULT_ZIP);
		return address;
	}
	...
	public void createAddress() throws Exception {
		...
		// Create the Address
		restAddressMockMvc.perform(post("/api/addresses").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(address))).andExpect(status().isCreated());
	...
	public void createAddressWithExistingId() throws Exception {
		...
		restAddressMockMvc.perform(post("/api/addresses").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(address))).andExpect(status().isBadRequest());
	...
	public void getAllAddresses() throws Exception {
		...
		restAddressMockMvc.perform(get("/api/addresses?sort=id,desc")).andExpect(status().isOk())
	...
	public void getAddress() throws Exception {
		...
		// Get the address
		restAddressMockMvc.perform(get("/api/addresses/{id}", address.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void updateAddress() throws Exception {
		...
		updatedAddress.addressLineOne(UPDATED_ADDRESS_LINE_ONE).addressLineTwo(UPDATED_ADDRESS_LINE_TWO)
				.city(UPDATED_CITY).state(UPDATED_STATE).zip(UPDATED_ZIP);

		restAddressMockMvc.perform(put("/api/addresses").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(updatedAddress))).andExpect(status().isOk());
	...
	public void deleteAddress() throws Exception {
		...
		// Get the address
		restAddressMockMvc
				.perform(delete("/api/addresses/{id}", address.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
	...
	public void searchAddress() throws Exception {
		...
		// Search the address
		restAddressMockMvc.perform(get("/api/_search/addresses?query=id:" + address.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.[*].id").value(hasItem(address.getId().intValue())))
				.andExpect(jsonPath("$.[*].addressLineOne").value(hasItem(DEFAULT_ADDRESS_LINE_ONE.toString())))
				.andExpect(jsonPath("$.[*].addressLineTwo").value(hasItem(DEFAULT_ADDRESS_LINE_TWO.toString())))
