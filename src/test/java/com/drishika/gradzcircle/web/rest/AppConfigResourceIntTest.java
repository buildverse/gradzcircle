...
public class AppConfigResourceIntTest {
	...
	public static AppConfig createEntity(EntityManager em) {
		AppConfig appConfig = new AppConfig().configName(DEFAULT_CONFIG_NAME).configValue(DEFAULT_CONFIG_VALUE);
		return appConfig;
	}
	...
	public void createAppConfig() throws Exception {
		...
		restAppConfigMockMvc.perform(post("/api/app-configs").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(appConfig))).andExpect(status().isCreated());
	...
	public void createAppConfigWithExistingId() throws Exception {
		...
		restAppConfigMockMvc.perform(post("/api/app-configs").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(appConfig))).andExpect(status().isBadRequest());
	...
	public void getAllAppConfigs() throws Exception {
		...
		restAppConfigMockMvc.perform(get("/api/app-configs?sort=id,desc")).andExpect(status().isOk())
	...
	public void getAppConfig() throws Exception {
		...
		// Get the appConfig
		restAppConfigMockMvc.perform(get("/api/app-configs/{id}", appConfig.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void getNonExistingAppConfig() throws Exception {
		// Get the appConfig
		restAppConfigMockMvc.perform(get("/api/app-configs/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
	}
	...
	public void updateAppConfig() throws Exception {
		...
		updatedAppConfig.configName(UPDATED_CONFIG_NAME).configValue(UPDATED_CONFIG_VALUE);

		restAppConfigMockMvc.perform(put("/api/app-configs").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(updatedAppConfig))).andExpect(status().isOk());
	...
	public void deleteAppConfig() throws Exception {
		...
		restAppConfigMockMvc
				.perform(delete("/api/app-configs/{id}", appConfig.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
