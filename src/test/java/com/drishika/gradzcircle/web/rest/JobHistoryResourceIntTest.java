...
public class JobHistoryResourceIntTest {
	...
	public static JobHistory createEntity(EntityManager em) {
		JobHistory jobHistory = new JobHistory().jobTitle(DEFAULT_JOB_TITLE).jobDescription(DEFAULT_JOB_DESCRIPTION)
				.noOfApplicants(DEFAULT_NO_OF_APPLICANTS).salary(DEFAULT_SALARY).jobStatus(DEFAULT_JOB_STATUS)
	...
	public void createJobHistory() throws Exception {
		...
		restJobHistoryMockMvc.perform(post("/api/job-histories").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(jobHistory))).andExpect(status().isCreated());
	...
	public void createJobHistoryWithExistingId() throws Exception {
		...
		restJobHistoryMockMvc.perform(post("/api/job-histories").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(jobHistory))).andExpect(status().isBadRequest());
	...
	public void getAllJobHistories() throws Exception {
		...
		restJobHistoryMockMvc.perform(get("/api/job-histories?sort=id,desc")).andExpect(status().isOk())
	...
	public void getJobHistory() throws Exception {
		...
		// Get the jobHistory
		restJobHistoryMockMvc.perform(get("/api/job-histories/{id}", jobHistory.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void updateJobHistory() throws Exception {
		...
		updatedJobHistory.jobTitle(UPDATED_JOB_TITLE).jobDescription(UPDATED_JOB_DESCRIPTION)
				.noOfApplicants(UPDATED_NO_OF_APPLICANTS).salary(UPDATED_SALARY).jobStatus(UPDATED_JOB_STATUS)
				.createDate(UPDATED_CREATE_DATE).originalJobCost(UPDATED_ORIGINAL_JOB_COST).jobCost(UPDATED_JOB_COST)
				.amountPaid(UPDATED_AMOUNT_PAID).totalAmountPaid(UPDATED_TOTAL_AMOUNT_PAID)
				.noOfApplicantsBought(UPDATED_NO_OF_APPLICANTS_BOUGHT)
				.removedFilterAmount(UPDATED_REMOVED_FILTER_AMOUNT)
				.additionalFilterAmount(UPDATED_ADDITIONAL_FILTER_AMOUNT).adminCharge(UPDATED_ADMIN_CHARGE)
				.adminChargeRate(UPDATED_ADMIN_CHARGE_RATE).upfrontDiscountRate(UPDATED_UPFRONT_DISCOUNT_RATE)
				.upfrontDiscountAmount(UPDATED_UPFRONT_DISCOUNT_AMOUNT).escrowAmountUsed(UPDATED_ESCROW_AMOUNT_USED)
				.escrowAmountAdded(UPDATED_ESCROW_AMOUNT_ADDED).paymentType(UPDATED_PAYMENT_TYPE)
				.hasBeenEdited(UPDATED_HAS_BEEN_EDITED).everActive(UPDATED_EVER_ACTIVE).canEdit(UPDATED_CAN_EDIT)
				.updateDate(UPDATED_UPDATE_DATE).createdBy(UPDATED_CREATED_BY).updatedBy(UPDATED_UPDATED_BY);

		restJobHistoryMockMvc.perform(put("/api/job-histories").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(updatedJobHistory))).andExpect(status().isOk());
	...
	public void deleteJobHistory() throws Exception {
		...
		restJobHistoryMockMvc
				.perform(delete("/api/job-histories/{id}", jobHistory.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
