...
public class CaptureQualificationResourceIntTest {
	...
	public void setup() {
		...
		final CaptureQualificationResource captureQualificationResource = new CaptureQualificationResource(
				captureQualificationRepository, captureQualificationSearchRepository);
	...
	public void createCaptureQualification() throws Exception {
		...
		restCaptureQualificationMockMvc
				.perform(post("/api/capture-qualifications").contentType(TestUtil.APPLICATION_JSON_UTF8)
		...
		CaptureQualification testCaptureQualification = captureQualificationList
				.get(captureQualificationList.size() - 1);
		...
		CaptureQualification captureQualificationEs = captureQualificationSearchRepository
				.findOne(testCaptureQualification.getId());
	...
	public void updateCaptureQualification() throws Exception {
		...
		CaptureQualification updatedCaptureQualification = captureQualificationRepository
				.findOne(captureQualification.getId());
		updatedCaptureQualification.qualificationName(UPDATED_QUALIFICATION_NAME);

		restCaptureQualificationMockMvc
				.perform(put("/api/capture-qualifications").contentType(TestUtil.APPLICATION_JSON_UTF8)
	...
	public void deleteCaptureQualification() throws Exception {
		...
		boolean captureQualificationExistsInEs = captureQualificationSearchRepository
				.exists(captureQualification.getId());
	...
	public void searchCaptureQualification() throws Exception {
		...
		restCaptureQualificationMockMvc
				.perform(get("/api/_search/capture-qualifications?query=id:" + captureQualification.getId()))
				.andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.[*].id").value(hasItem(captureQualification.getId().intValue())))
				.andExpect(jsonPath("$.[*].qualificationName").value(hasItem(DEFAULT_QUALIFICATION_NAME.toString())));
	}
