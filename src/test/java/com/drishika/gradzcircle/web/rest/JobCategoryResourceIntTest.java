...
public class JobCategoryResourceIntTest {
	...
	public void setup() {
		...
		final JobCategoryResource jobCategoryResource = new JobCategoryResource(jobCategoryRepository,
				jobCategorySearchRepository);
	...
	public static JobCategory createEntity(EntityManager em) {
		JobCategory jobCategory = new JobCategory().jobCategory(DEFAULT_JOB_CATEGORY);
		return jobCategory;
	}
	...
	public void createJobCategory() throws Exception {
		...
		restJobCategoryMockMvc.perform(post("/api/job-categories").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(jobCategory))).andExpect(status().isCreated());
		...
		assertThat(jobCategoryEs).isEqualToComparingFieldByField(testJobCategory);
	}
	...
	public void createJobCategoryWithExistingId() throws Exception {
		...
		restJobCategoryMockMvc.perform(post("/api/job-categories").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(jobCategory))).andExpect(status().isBadRequest());
	...
	public void getAllJobCategories() throws Exception {
		...
		restJobCategoryMockMvc.perform(get("/api/job-categories?sort=id,desc")).andExpect(status().isOk())
	...
	public void getJobCategory() throws Exception {
		...
		// Get the jobCategory
		restJobCategoryMockMvc.perform(get("/api/job-categories/{id}", jobCategory.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void updateJobCategory() throws Exception {
		...
		updatedJobCategory.jobCategory(UPDATED_JOB_CATEGORY);

		restJobCategoryMockMvc.perform(put("/api/job-categories").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(updatedJobCategory))).andExpect(status().isOk());
	...
	public void deleteJobCategory() throws Exception {
		...
		restJobCategoryMockMvc
				.perform(delete("/api/job-categories/{id}", jobCategory.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
