...
public class FilterResourceIntTest {
	...
	public static Filter createEntity(EntityManager em) {
		Filter filter = new Filter().filterName(DEFAULT_FILTER_NAME).filterCost(DEFAULT_FILTER_COST)
				.comments(DEFAULT_COMMENTS).matchWeight(DEFAULT_MATCH_WEIGHT);
		return filter;
	}
	...
	public void createFilter() throws Exception {
		...
		// Create the Filter
		restFilterMockMvc.perform(post("/api/filters").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(filter))).andExpect(status().isCreated());
	...
	public void createFilterWithExistingId() throws Exception {
		...
		restFilterMockMvc.perform(post("/api/filters").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(filter))).andExpect(status().isBadRequest());
	...
	public void getAllFilters() throws Exception {
		...
		restFilterMockMvc.perform(get("/api/filters?sort=id,desc")).andExpect(status().isOk())
	...
	public void getFilter() throws Exception {
		...
		// Get the filter
		restFilterMockMvc.perform(get("/api/filters/{id}", filter.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void getNonExistingFilter() throws Exception {
		// Get the filter
		restFilterMockMvc.perform(get("/api/filters/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
	}
	...
	public void updateFilter() throws Exception {
		...
		updatedFilter.filterName(UPDATED_FILTER_NAME).filterCost(UPDATED_FILTER_COST).comments(UPDATED_COMMENTS)
				.matchWeight(UPDATED_MATCH_WEIGHT);

		restFilterMockMvc.perform(put("/api/filters").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(updatedFilter))).andExpect(status().isOk());
	...
	public void deleteFilter() throws Exception {
		...
		// Get the filter
		restFilterMockMvc.perform(delete("/api/filters/{id}", filter.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
	...
	public void searchFilter() throws Exception {
		...
		// Search the filter
		restFilterMockMvc.perform(get("/api/_search/filters?query=id:" + filter.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.[*].id").value(hasItem(filter.getId().intValue())))
				.andExpect(jsonPath("$.[*].filterName").value(hasItem(DEFAULT_FILTER_NAME.toString())))
				.andExpect(jsonPath("$.[*].filterCost").value(hasItem(DEFAULT_FILTER_COST.doubleValue())))
